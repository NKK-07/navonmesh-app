...
export function renderShelfLifePage(state, currentLang) {
  ...
  return `
    <div style="display: flex; flex-direction: column; gap: 24px;">
      ...
      <div class="card" style="background: linear-gradient(135deg, #231F1C 0%, #241F1B 100%); color: white; padding: 32px; display: flex; flex-direction: column; gap: 24px;">
        
        <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 16px;">
          <div style="display: flex; align-items: center; gap: 16px;">
            <span style="font-size: 54px;">${crop.icon}</span>
            <div>
              <h2 style="font-size: 28px; color: #FFFDFA;">${crop.name.toUpperCase()}</h2>
              <span style="color: #8C7F73; font-size: 14px;">Stored: ${state.produceWeightKg} kg | Stored Since: 02 Sep 2026</span>
            </div>
          </div>
        ...
        <div style="display: flex; flex-direction: column; gap: 8px;">
          <span style="font-size: 13px; color: #8C7F73; font-weight: 800; letter-spacing: 1px;">ESTIMATED REMAINING SHELF LIFE</span>
          <div style="font-size: 64px; font-weight: 900; font-family: var(--font-display); color: #04785C; line-height: 1;">
            ${days} DAYS
          </div>
        </div>
        ...
        <div style="display: flex; flex-direction: column; gap: 8px;">
          <div style="display: flex; justify-content: space-between; font-size: 13px; font-weight: 700;">
            <span>Freshness Grade: <strong style="color: #04785C;">${quality}</strong></span>
            <span>${pctRemaining}% Viability Remaining</span>
          </div>
          <div style="width: 100%; height: 16px; background: #3C352F; overflow: hidden; position: relative;">
            <div style="width: ${pctRemaining}%; height: 100%; background: linear-gradient(90deg, #04785C, #04785C);"></div>
          </div>
        </div>
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 16px; margin-top: 10px;">
          ...
          <div style="background: rgba(255,255,255,0.06); padding: 16px; border: 1px solid rgba(255,255,255,0.1);">
            <span style="font-size: 12px; color: #8C7F73; font-weight: 700;">TEMP EXPOSURE</span>
            <div style="font-size: 18px; font-weight: 800; color: white; margin-top: 4px;">${factors.tempExposure}</div>
            <span style="font-size: 11px; color: #8C7F73;">Mean: ${state.temperature.toFixed(1)}°C</span>
          </div>

          <div style="background: rgba(255,255,255,0.06); padding: 16px; border: 1px solid rgba(255,255,255,0.1);">
            <span style="font-size: 12px; color: #8C7F73; font-weight: 700;">HUMIDITY STATUS</span>
            <div style="font-size: 18px; font-weight: 800; color: white; margin-top: 4px;">${factors.humidityStatus}</div>
            <span style="font-size: 11px; color: #8C7F73;">Target RH: ${crop.humidityTarget}%</span>
          </div>

          <div style="background: rgba(255,255,255,0.06); padding: 16px; border: 1px solid rgba(255,255,255,0.1);">
            <span style="font-size: 12px; color: #8C7F73; font-weight: 700;">STORAGE QUALITY</span>
            <div style="font-size: 18px; font-weight: 800; color: #04785C; margin-top: 4px;">${factors.qualityGrade}</div>
            <span style="font-size: 11px; color: #8C7F73;">Zero Chilling Injury</span>
          </div>

        </div>

        <div style="font-size: 12px; color: #8C7F73; background: rgba(0,0,0,0.3); padding: 12px; border: 1px dashed rgba(255,255,255,0.2);">
