...
export function renderSystemHealthPage(state, currentLang) {
  ...
  const sensorRows = [
    { name: 'ESP32 Dual-Core MCU', desc: 'Main System Controller & Sensor Bus', status: '🟢 Healthy', detail: s.esp32.temp },
    { name: 'SHT31 Temperature Sensor', desc: 'Chamber Air Temp (Accuracy ±0.3°C)', status: '🟢 Calibrated', detail: 'Calibrated 01 Aug' },
    { name: 'SHT31 Humidity Sensor', desc: 'Chamber Relative Humidity (Accuracy ±2%)', status: '🟢 Calibrated', detail: 'Calibrated 01 Aug' },
    { name: '48V LiFePO4 BMS', desc: 'Battery Management System', status: '🟢 Healthy', detail: `${s.batteryBms.healthPct}% Health` },
    { name: 'BLDC Variable Compressor', desc: 'DC Inverter Refrigeration Loop', status: '🟢 Healthy', detail: s.compressor.runtimeHours },
    { name: 'Magnetic Reed Door Switch', desc: 'Door Seal Contact Switch', status: state.doorOpen ? '🟡 Open' : '🟢 Healthy', detail: 'Real-time' },
    { name: '4-Point Load Cell', desc: 'Produce Weight Strain Gauge', status: '🟢 Healthy', detail: s.loadCell.accuracy },
    { name: 'Electrochemical Ethylene Sensor', desc: 'Ripening Gas Detector', status: '🟢 Healthy', detail: 'Normal' },
    { name: 'SX1276 LoRa Transceiver', desc: 'Long Range 868MHz Radio', status: '🟢 Healthy', detail: '8km Range' }
  ];
  ...
  return `
    <div style="display: flex; flex-direction: column; gap: 24px;">
      ...
      <div>
        <h1 style="font-size: 28px; margin-bottom: 4px;">🩺 ${getTranslation(currentLang, 'navHealth')}</h1>
      ...
      <div class="card" style="background: var(--bg-card); border-left: 6px solid var(--agri-green); display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 16px;">
        <div>
          <strong style="font-size: 16px;">📅 Hardware Maintenance Schedule</strong>
